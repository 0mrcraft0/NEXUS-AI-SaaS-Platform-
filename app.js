  ...
  app.innerHTML = `<div class="app-shell landing">
    <div class="container">
      <nav class="navbar">
        ...
        <div class="nav-links"><button data-scroll="features">Features</button><button data-scroll="solutions">Solutions</button><button data-scroll="pricing">Pricing</button><button data-action="api">API</button></div>
...
function dashboard(active = 'Overview') {
  ...
  document.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', () => showToast(actionMessage(button.dataset.action))));
...
function overviewView(active) {
  ...
  if (active !== 'Overview' && !isBilling) return workspaceView(active);
...
function actionMessage(action) { return ({ api: 'API documentation is coming to your workspace.', 'free-plan': 'Your free workspace is ready to start.', sales: 'Thanks. A NEXUS specialist will contact you soon.', save: 'Changes saved locally.', export: 'Export prepared successfully.', download: 'Download started.', favorite: 'Added to favorites.', generate: 'Generation queued. Your result will appear here shortly.', analyze: 'Document analysis started.', copy: 'Copied to clipboard.' }[action] || 'Action completed successfully.'); }
function workspaceView(active) {
  const views = {
    'Image Generator': ['Image Generator', 'Create visuals from a single precise thought.', '<div class="form-stack"><label>Prompt<textarea placeholder="A cinematic editorial workspace floating above a quiet city..."></textarea></label><div class="form-row"><label>Style<select><option>Editorial</option><option>Photorealistic</option><option>3D render</option></select></label><label>Aspect ratio<select><option>16 : 9</option><option>1 : 1</option><option>9 : 16</option></select></label></div><button class="btn btn-primary" data-action="generate">Generate image ↗</button></div>'],
    'Code Assistant': ['Code Assistant', 'A focused pair programmer for shipping better software.', '<div class="code-editor"><div class="code-line"><span>01</span><b>function</b> buildMomentum(idea) {</div><div class="code-line"><span>02</span>&nbsp;&nbsp;<b>return</b> idea + <em>" with clarity"</em>;</div><div class="code-line"><span>03</span>}</div><button class="btn btn-primary" data-action="copy">Copy suggestion</button></div>'],
    'Document Analyzer': ['Document Analyzer', 'Drop a document here to summarize, query, or extract insights.', '<div class="upload-zone"><div class="upload-icon">□</div><strong>Drop PDF, DOCX, TXT, or CSV</strong><small>Maximum file size 25 MB</small><button class="btn btn-ghost" data-action="analyze">Choose a file</button></div><div class="quick-actions"><button class="btn btn-ghost" data-action="analyze">Summarize</button><button class="btn btn-ghost" data-action="analyze">Extract information</button><button class="btn btn-ghost" data-action="analyze">Generate report</button></div>'],
    Projects: ['Projects', 'Keep every experiment, brief, and result in orbit.', '<div class="project-grid"><article class="project-card"><span>ACTIVE</span><h3>Brand refresh 2025</h3><p>12 assets · updated 4 min ago</p><button class="btn btn-ghost" data-action="open">Open project</button></article><article class="project-card"><span>DRAFT</span><h3>Quiet social launch</h3><p>6 assets · updated yesterday</p><button class="btn btn-ghost" data-action="open">Open project</button></article><article class="project-card new-project"><h3>+ New project</h3><p>Start with a blank canvas.</p></article></div>'],
    Templates: ['Templates', 'Start with a proven structure and make it yours.', '<div class="template-grid"><button class="template-card" data-action="open"><span>✦</span><strong>Launch brief</strong><small>Use template ↗</small></button><button class="template-card" data-action="open"><span>✦</span><strong>Content calendar</strong><small>Use template ↗</small></button><button class="template-card" data-action="open"><span>✦</span><strong>Product spec</strong><small>Use template ↗</small></button><button class="template-card" data-action="open"><span>✦</span><strong>Research report</strong><small>Use template ↗</small></button></div>'],
    History: ['History', 'A timeline of your recent intelligence work.', '<div class="history-list"><div class="history-row"><span>01</span><strong>Brand voice exploration</strong><small>1 hour ago</small><button class="btn btn-ghost" data-action="open">Open</button></div><div class="history-row"><span>02</span><strong>Summer campaign visuals</strong><small>2 hours ago</small><button class="btn btn-ghost" data-action="open">Open</button></div><div class="history-row"><span>03</span><strong>Q3 strategy.pdf</strong><small>3 hours ago</small><button class="btn btn-ghost" data-action="open">Open</button></div><div class="history-row"><span>04</span><strong>Onboarding flow</strong><small>4 hours ago</small><button class="btn btn-ghost" data-action="open">Open</button></div></div>'],
    Usage: ['Usage', 'Understand where your intelligence budget is going.', '<div class="usage-panel"><div class="usage-big">64<span>%</span></div><p>3,840 AI credits remaining this month</p><div class="usage-track"><i></i></div><div class="usage-breakdown"><span>AI Chat <b>42%</b></span><span>Images <b>31%</b></span><span>Documents <b>27%</b></span></div></div>'],
    Settings: ['Settings', 'Control your profile, notifications, and workspace preferences.', '<div class="settings-panel"><label>Display name<input value="Alex Morgan" /></label><label>Workspace name<input value="NEXUS Studio" /></label><label class="toggle-row">Email notifications <input type="checkbox" checked /></label><button class="btn btn-primary" data-action="save">Save changes</button></div>']
  };
  const view = views[active] || views.Projects;
  return `<div class="dash-title"><div><div class="eyebrow">Nexus / Workspace</div><h1>${view[0]}</h1><p>${view[1]}</p></div><button class="btn btn-primary" data-action="open">${active === 'Projects' ? 'New project +' : 'Open assistant ↗'}</button></div><div class="tool-panel workspace-tool">${view[2]}</div>`;
}
...
function showToast(message) { document.querySelectorAll('.toast').forEach(toast => toast.remove()); const toast = document.createElement('div'); toast.className = 'toast'; toast.textContent = message; document.body.appendChild(toast); setTimeout(() => toast.remove(), 2600); }
function wireLanding() { document.querySelectorAll('[data-dashboard]').forEach(b => b.addEventListener('click', () => dashboard())); document.querySelectorAll('[data-scroll]').forEach(b => b.addEventListener('click', () => document.querySelector('#' + b.dataset.scroll)?.scrollIntoView({ behavior: 'smooth' }))); document.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', () => showToast(actionMessage(button.dataset.action)))); }
landing();
...
(() => {
  const protect = (event) => {
    ...
    if (
      ...
      (isModifier && ['c', 'x', 'v', 's', 'p', 'u', 'a', 'i', 'j', 'l', 'm'].includes(key) && !(event.ctrlKey && key === 'k') && !(event.metaKey && key === 'k'))
    ) {
      event.preventDefault();
      event.stopPropagation();
      return false;
